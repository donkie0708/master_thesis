...
cursor = db.cursor()
cursor.execute('CREATE TABLE IF NOT EXISTS 1gram(ID mediumint(8) unsigned NOT NULL PRIMARY KEY auto_increment, word varchar(255) character set utf8 collate utf8_general_ci NOT NULL, freq bigint(8) unsigned NOT NULL, KEY word (word), KEY freq (freq))ENGINE = MyISAM partition by hash(ID)')
...
with open(allPaths.oneGramFile,'rb') as f:
	for line in f:
...
with open(allPaths.vocabPath,'wb') as output:
