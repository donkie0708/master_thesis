...
def findHead():
	...
	for i in tempFiles[(numList[0]+numList[1]+numList[2]):(numList[0]+numList[1]+numList[2]+numList[3])]:
		with open(i,'rb') as r:
			string = r.readline()
			string ='\t'.join((string[:-1],i[-8:]))		
			firstLine.append(string)
	
	for i in tempFiles[(numList[0]+numList[1]+numList[2]+numList[3]):(numList[0]+numList[1]+numList[2]+numList[3]+numList[4])]:
		with open(i,'rb') as r:
			string = r.readline()
			string ='\t'.join((string[:-1],i[-8:]))		
			firstLine.append(string)
	...
	for i in tempFiles[(numList[0]+numList[1]+numList[2]+numList[3]+numList[4]):]:
		with open(i,'rb') as r:
			string = r.readline()
			string ='\t'.join((string[:-1],i[-8:]))	
			firstLine.append(string)
